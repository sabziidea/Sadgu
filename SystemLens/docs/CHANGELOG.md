...
- Fixed Preview behaviour for sandboxed/embedded browsers: Preview now opens a blank tab synchronously and writes the standalone preview HTML into it; if the browser blocks the new tab, SystemLens falls back to the in-app expanded preview instead of failing. Project persistence also falls back to in-memory storage when `localStorage` is unavailable.
